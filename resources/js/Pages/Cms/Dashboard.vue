...
const cards = computed(() => [
    { label: "Total Pengguna", value: props.totalUsers, icon: "Users", desc: "User terdaftar di sistem" },
    { label: "User Aktif Undangan", value: props.activeUsers, icon: "CircleCheck", desc: "User yang memiliki undangan" },
    { label: "Total Undangan (Weddings)", value: props.totalWeddings, icon: "Heart", desc: "Pernikahan yang didaftarkan" },
    { label: "Template Kustom", value: props.totalCustomTemplates, icon: "Template", desc: "Template kustom buatan admin" },
]);
...
<template>
    ...
    >
        <div class="bg-card border border-border rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-150">
            <!-- Modal Header -->
            <div class="p-5 border-b border-border flex items-center justify-between bg-emerald-900/10 dark:bg-emerald-950/20">
                <div>
                    <h3 class="text-lg font-bold text-foreground">Detail Undangan: {{ selectedWedding.label }}</h3>
                    <p class="text-xs text-muted-foreground font-mono">ID: {{ selectedWedding.slug }}</p>
                </div>
                <button
                    @click="closeModal"
            ...
            <div class="p-6 overflow-y-auto space-y-6">
                <!-- Bride & Groom Info -->
                <div class="space-y-2">
                    <h4 class="text-sm font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                        <IconComponent name="Heart" class="h-4 w-4 text-emerald-600" />
                        Pasangan Pengantin
                    </h4>
                    <div class="grid grid-cols-2 gap-4 bg-muted/30 rounded-xl p-4 border border-border/50">
                        <div class="space-y-1">
                            <span class="text-[10px] text-muted-foreground uppercase font-bold">Mempelai Pria</span>
                            <p class="font-bold text-foreground text-sm">
                                {{ selectedWedding.couple?.groom_full_name || 'Belum diisi' }}
                            </p>
                            <p class="text-xs text-muted-foreground italic" v-if="selectedWedding.couple?.groom_nickname">
                                ({{ selectedWedding.couple.groom_nickname }})
                            </p>
                        </div>
                        <div class="space-y-1 border-l border-border pl-4">
                            <span class="text-[10px] text-muted-foreground uppercase font-bold">Mempelai Wanita</span>
                            <p class="font-bold text-foreground text-sm">
                                {{ selectedWedding.couple?.bride_full_name || 'Belum diisi' }}
                            </p>
                            <p class="text-xs text-muted-foreground italic" v-if="selectedWedding.couple?.bride_nickname">
                ...
                <div class="space-y-2">
                    <h4 class="text-sm font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                        <IconComponent name="CalendarEvent" class="h-4 w-4 text-emerald-600" />
                        Agenda & Acara
                    </h4>
                    <div v-if="selectedWedding.events && selectedWedding.events.length > 0" class="space-y-3">
                        <div
                        ...
                        >
                            <div class="flex justify-between items-center mb-1">
                                <span class="font-bold text-foreground text-sm">{{ ev.name }}</span>
                                <Badge class="bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">Jadwal</Badge>
                            </div>
                            <p class="text-muted-foreground">📅 {{ formatEventDate(ev.date) }}</p>
                            <p class="text-muted-foreground">⏰ {{ ev.start_time || '00:00' }} WIB s/d selesai</p>
                            <p class="text-muted-foreground">📍 {{ ev.location_name || 'Lokasi belum dispesifikasikan' }}</p>
                        </div>
                    </div>
                    <div v-else class="text-xs text-center py-4 bg-muted/20 rounded-xl border border-border/50 text-muted-foreground">
                ...
                <div class="space-y-3">
                    <h4 class="text-sm font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                        <IconComponent name="ShieldLock" class="h-4 w-4 text-emerald-600" />
                        ...
                        >
                            <span
                                :class="[
                                    ...
                                    isFeatureEnabled(selectedWedding, feat.key) ? 'bg-emerald-600' : 'bg-red-500'
                                ]"
                            >
                                <IconComponent :name="isFeatureEnabled(selectedWedding, feat.key) ? 'Check' : 'X'" class="h-3 w-3 stroke-[3]" />
                            </span>
                            <span :class="isFeatureEnabled(selectedWedding, feat.key) ? 'font-medium text-foreground' : 'text-muted-foreground line-through'">
                                {{ feat.label }}
                            </span>
                        </div>
                    </div>
            ...
            <div class="p-4 border-t border-border flex justify-end bg-muted/20">
                <Button variant="outline" @click="closeModal">Tutup Detail</Button>
            </div>
        </div>
    </div>
</template>
